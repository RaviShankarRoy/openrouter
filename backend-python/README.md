...
|---|---|---|
| Domain-Driven Design | `app/service/domain/` + `app/service/` | Map to business capabilities |
| Repository | `app/repository/` | Hide SQLAlchemy from use cases |
| Unit of Work | `app/repository/uow.py` | One transaction per use case |
| Service Layer | `app/service/` | Thin orchestration over the domain |
| Adapter | `app/repository/providers/`, `storage/`, `billing/` | Translate to/from external APIs |
| Factory + Registry | `app/repository/providers/registry.py` | Plugin architecture for new providers |
| Strategy | `app/service/routing/strategies.py` | Cost/latency/quality interchangeable |
| Pipeline / Chain of Responsibility | `app/service/guardrails/pipeline.py` | DRD §14.3 |
...
| Pub/Sub | `app/repository/events/bus.py` | Decouple metering from request path |
...
The gRPC server boots inside the FastAPI process when `app.repository.grpc.gen.*` stubs are present (run `make proto` from the monorepo root once codegen is wired).
...
All config is environment-driven, validated at startup by `app/shared/config.py` (`pydantic-settings`). See `../.env.example` for every variable and its default. Secrets are never read from files at request time — they're loaded once into a frozen `Settings` singleton.
