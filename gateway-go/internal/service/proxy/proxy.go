...
import (
	...
	"github.com/openrouter/gateway-go/internal/repository/adapters"
...
// every target failed.
func (p *Proxy) ChatCompletion(ctx context.Context, modelID string, body []byte, stream bool, w http.ResponseWriter) error {
	primary, fallback, ok := p.router.Resolve(modelID)
	if !ok {
		return fmt.Errorf("unknown model %q", modelID)
	}
	chain := append([]model.Target{primary}, fallback...)
